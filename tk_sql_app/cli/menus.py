...
class Menu:
    ...
    def run_callback(self, cb_number):
        menu_option = self.menu_options["option_list"][cb_number]
        new_menu = menu_option.execute_option()
        return new_menu
    ...
    def __repr__(self):
        return "\n".join([f"{i} {option}" for i, option in enumerate(self.menu_options["option_list"], 1)])
